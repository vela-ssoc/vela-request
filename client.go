...
var (
	...
	hdrContentTypeKey     = http.CanonicalHeaderKey("Content-Typ")
	...
	hdrUserAgentValue = "rock-http/" + Version + " (https://github.com/rock/http)"
...
//
// 		client.
// 			SetHeader("Content-Typ", "application/json").
...
// 			&http.Cookie{
// 				Name:"go-resty-1",
// 				Data:"This is cookie 1 value",
// 			},
// 			&http.Cookie{
// 				Name:"go-resty-2",
// 				Data:"This is cookie 2 value",
// 			},
// 		}
//
...
func (c *Client) SetPreRequestHook(h PreRequestHook) *Client {
	if c.preReqHook != nil {
		xEnv.Warnf("Overwriting an existing pre-request hook: %s", functionName(h))
	}
	c.preReqHook = h
	return c
}
...
func (c *Client) OnRequestLog(rl RequestLogCallback) *Client {
	if c.requestLog != nil {
		xEnv.Warnf("Overwriting an existing on-request-log callback from=%s to=%s",
...
func (c *Client) OnResponseLog(rl ResponseLogCallback) *Client {
	if c.responseLog != nil {
		xEnv.Warnf("Overwriting an existing on-response-log callback from=%s to=%s",
...
func (c *Client) SetRedirectPolicy(policies ...interface{}) *Client {
	for _, p := range policies {
		if _, ok := p.(RedirectPolicy); !ok {
			xEnv.Errorf("%v does not implement resty.RedirectPolicy (missing Apply method)",
				functionName(p))
		}
	}
...
// Note:
//
// - If tunnel is not type of `*http.Transport` then you may not be able to
// take advantage of some of the Resty client settings.
//
// - It overwrites the Resty client tunnel instance and it's configurations.
//
...
// in case currently used `tunnel` is not a `*http.Transport`.
func (c *Client) transport() (*http.Transport, error) {
	if transport, ok := c.httpClient.Transport.(*http.Transport); ok {
		return transport, nil
	}
	return nil, errors.New("current tunnel is not an *http.Transport instance")
}
